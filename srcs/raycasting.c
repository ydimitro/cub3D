...
{
	if (data->ray.side == 0)
		data->ray.perp_wall_dist = (data->ray.map_x - data->player.x + (1 - data->ray.step_x) / 2) / data->ray.dir_x;
	else
		data->ray.perp_wall_dist = (data->ray.map_y - data->player.y + (1 - data->ray.step_y) / 2) / data->ray.dir_y;
...
void cast_single_ray(t_data *data, int x)
{
	...
	data->ray.dir_x = data->player.direction + cam_x;
