...
void	move_player(t_wall *wall, int indentifier)
{
	...
	{
		wall->p_c_x = wall->p_m[0];
		wall->p_c_y = wall->p_m[1];
	}
	if (indentifier == S_KEY)
	{
		wall->p_c_x = wall->p_m[6];
		wall->p_c_y = wall->p_m[7];
	}
	if (indentifier == A_KEY)
	{
		wall->p_c_x = wall->p_m[4];
		wall->p_c_y = wall->p_m[5];
	}
	if (indentifier == D_KEY)
	{
		wall->p_c_x = wall->p_m[2];
		wall->p_c_y = wall->p_m[3];
	}
}
...
{
	wall->p_c_x = TILE + (wall->main->p_x * (TILE)) + ((TILE) / 2);
	wall->p_c_y = TILE + (wall->main->p_y * (TILE)) + ((TILE) / 2);
	play_music();
